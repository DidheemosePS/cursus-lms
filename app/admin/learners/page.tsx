...
import { getLearners } from "@/dal/admin/learners.dal";
import { AdminLearnerTable } from "./components/learner-table";
import LearnerFilters from "./components/learner-filters";
import InviteLearnerModal from "./components/invite-modal";
...
export default async function Page({ searchParams }: PageProps) {
  ...
  const { learners, total, pageSize, counts } = await getLearners({
  ...
  return (
    <main className="@container min-h-[calc(100dvh-4rem)] px-4 py-8 md:px-12 flex flex-col gap-8">
      ...
      {/* Filters */}
      <Suspense>
        <LearnerFilters counts={counts} />
      </Suspense>

      {/* Table */}
      <AdminLearnerTable learners={learners} />
